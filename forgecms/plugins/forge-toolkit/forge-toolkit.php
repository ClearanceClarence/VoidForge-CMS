 ...
 * Description: The ultimate content toolkit for Forge CMS — shortcodes, components, security tools, utilities, and developer helpers
 * Version: 3.0.0
 * Author: Forge CMS
 */
...
// Register API endpoint for security salts
add_action('init', function() {
    $requestUri = $_SERVER['REQUEST_URI'] ?? '';
    
    // Handle /api/salts endpoint
    if (preg_match('#/api/salts(?:\.php)?/?$#', $requestUri)) {
        header('Content-Type: text/plain; charset=utf-8');
        header('Cache-Control: no-cache, no-store, must-revalidate');
        header('X-Forge-CMS: Salt Generator v1.0');
    ...
    if (preg_match('#/api/salts/json/?$#', $requestUri)) {
...
function forge_generate_salts() {
    ...
    $output = "/**\n";
    $output .= " * Forge CMS Security Keys and Salts\n";
...
register_tag('sitename', function() { return esc(getOption('site_title', 'Forge CMS')); }, ['has_content' => false]);
...
Forge CMS is a modern, lightweight content management system built with pure PHP. It features a clean admin interface, powerful plugin system, custom post types, custom fields, and a flexible theming engine. No frameworks, no dependencies — just fast, reliable content management.
{/accordion-item}
...
Yes! Forge CMS is 100% free and open-source under the MIT license. You can use it for personal projects, client work, or commercial applications without any restrictions.
{/accordion-item}
{/accordion}
...
Learn the basics of Forge CMS and create your first page in minutes. Our quick-start guide walks you through installation and setup.
{button href="#" style="outline" size="sm"}Read Guide{/button}
{/card}
{card title="Customization" icon="palette"}
Discover how to customize themes, add plugins, and extend functionality. Make Forge CMS truly yours.
...
Forge CMS is exactly what I've been looking for. Clean code, fast performance, and incredibly easy to extend. It's become my go-to CMS for client projects.
{/testimonial}
</div>
<div>
{testimonial author="Mike Chen" role="Agency Owner" rating="5"}
We've moved all our client sites to Forge CMS. The custom post types and fields feature is a game-changer. Our development time has been cut in half!
{/testimonial}
</div>
{/grid}
